...
left join {{ ref('stg_etl_duplicate_trace') }} dtr on tr.block_hash = dtr.block_hash 
                                                    ...
                                                    and MD5(ARRAY_TO_STRING(ARRAY(SELECT CAST(ta AS STRING) FROM UNNEST(tr.trace_address) ta), '.')) = dtr.trace_hash
...
left join {{ ref('stg_etl_faulty_trace') }} ftr on tr.block_hash = ftr.block_hash 
                                                ...
                                                and MD5(ARRAY_TO_STRING(ARRAY(SELECT CAST(ta AS STRING) FROM UNNEST(tr.trace_address) ta), '.')) = ftr.trace_hash
